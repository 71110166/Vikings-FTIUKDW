<?php
    ...
    if (isset($_POST['submitted'])) {
        ...
        $stmt = $dbc->prepare('SELECT * FROM admin WHERE username = ? LIMIT 1');
        ...
        if ($result->num_rows == 1) {
            $hpass = $result->fetch_assoc()['password'];

            // cek masukan password dengan hash password di db
            if (fnPasswordCheck($pass, $hpass)) {
                // Login berhasil
                // tutup koneksi db
                $stmt->close();
                fnCloseDB($dbc);
